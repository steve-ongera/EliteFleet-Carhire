...
def add_cart(request, product_id):
    ...
    car = Cars.objects.get(id=product_id) #get the car
    ...
    if current_user.is_authenticated:
        ...
        if request.method == 'POST':
            for item in request.POST:
                ...
                try:
                    variation = Variation.objects.get(car=car, variation_category__iexact=key, variation_value__iexact=value)
        ...
        if is_cart_item_exists:
            cart_item = CartItem.objects.filter(car=car, user=current_user)
            ex_var_list = []
            id = []
            ...
            if product_variation in ex_var_list:
                ...
                item_id = id[index]
                item = CartItem.objects.get(car=car, id=item_id)
                item.quantity += 1
                item.save()
...
def remove_cart(request, product_id, cart_item_id):
    ...
    car = get_object_or_404(Cars, id=product_id)
    try:
...
def checkout(request, total=0, quantity=0, cart_items=None):
    ...
    context = {
        ...
        'tax'       : tax,
        'grand_total': grand_total,
        'locations': locations,  # Add locations to the context
        'countys': countys,  # Add county to the context
    }
